...
public struct BottomSheetView<Content: View>: View {
    ...
    ///   - configuration: A binding to a `SheetConfiguration` object that defines the appearance and behavior of the sheet.
    ...
    var onDragChange: (DragGesture.Value) -> Void = { _ in }
    ...
    private func sheetView() -> some View {
        GeometryReader { geometry in
            ...
            .frame(
                width: adjustedWidth(for: geometry.size.width),
            ...
            .offset(y: viewModel.yOffset(for: translation))
            .padding(style.padding)
            .gesture(onDrag(from: geometry.size.height))
            .animation(
                style.animation.value,
                value: SheetAnimations.Trigger(
